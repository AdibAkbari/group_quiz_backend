...
describe('User owns no quizzes', () => {
    ...
    test('One quiz created on system', () => {
        let user2 = adminAuthRegister('email1@gmail.com', 'password2', 'FirstnameB', 'LastnameB');
...
describe('User does own quizzes', () => {
    ...
    test('user owns two quizzes', () => {
        let quiz2 = adminQuizCreate(user.authUserId, 'Dogs', 'A quiz about dogs');
        let expected = {
            quizzes: [
                {
                    quizId: quiz.quizId,
    ...
    test('user owns multiple quizzes', () => {
        let user2 = adminAuthRegister('email1@gmail.com', 'password2', 'FirstnameB', 'LastnameB');
        let quiz2 = adminQuizCreate(user.authUserId, 'Dogs', 'A quiz about dogs');
        let quiz3 = adminQuizCreate(user2.authUserId, 'Birds', 'A quiz about birds');
        let quiz4 = adminQuizCreate(user.authUserId, 'Ducks', 'A quiz about ducks');
        let quiz5 = adminQuizCreate(user.authUserId, 'Lizards', 'A quiz about lizards');
        let quiz6 = adminQuizCreate(user2.authUserId, 'Goats', 'A quiz about goats');
